...
namespace TrungKien.Core.UI
{
    ...
    {
        [SerializeField] TextMeshProUGUI txtScore, txtDebugFPS, txtTimeLimit;
        ...
        [SerializeField] Button butSetting, butResetFPS;
        [SerializeField] GameObject objDebugFPS;
        void Start()
        {
        ...
        void Update()
        {
            if (GameManager.Instance.IsCheat)
            {
        ...
        {
            if (listUISandBowl != null)
            {
                for (int i = 0; i < listUISandBowl.Count; i++)
                {
                    PoolingSystem.Despawn(listUISandBowl[i]);
                }
            }
        ...
        public void SetUpCacheSandBowl(int amount)
        {
            if (listUICacheSandBowl != null)
            {
                ...
                {
                    PoolingSystem.Despawn(listUICacheSandBowl[i]);
                }
            }
        ...
        void ShowCheat()
        {
            objDebugFPS.SetActive(GameManager.Instance.IsCheat);
        }
        ...
        public void VisualShareSandCacheBowlToMainBowl(int mainBowlIndex, int cacheBowlIndex, int indexColor, float timeDespawn = 1.5f)
        {
            ...
            sandLine.SetUp(posCache, sandLine.TF.up, listUISandBowl[mainBowlIndex].TF, 0.5f, LevelControl.Instance.GetColor(indexColor), timeDespawn);
            sandLine.TF.SetParent(transform);
        ...
        {
            cameraControl.CamZoom(value, rangeZoom);
        }
